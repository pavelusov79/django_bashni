...
def parse_flats():
    ...
    with open(logger_path, 'a') as f:
        f.write(f"OK *** {datetime.datetime.now(pytz.timezone('Asia/Vladivostok')).strftime('%d.%m.%y %H:%M')} *** спарсено {db.flats.count_documents({'living_complex_name': 'ЖК Фрегат2'})} квартир с fregat.py\n")
    return db
